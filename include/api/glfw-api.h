 ...
 * @param [i] buf framebuffer id
 */
#define __glCreateAndBindFramebuf(buf)                        {  __glCreateFramebuf(buf); glBindFramebuffer(GL_FRAMEBUFFER, buf); }
 ...
 * @param [i] tex 纹理ID引用
 */
#define __glCreateTexture2D(tex)                              { glGenTextures(1, &tex); }
 ...
 * @param [i] tex 纹理ID引用
 */
#define __glCreateAndBindTexture2D(tex)                       { __glCreateTexture2D(tex); glBindTexture(GL_TEXTURE_2D, texture); }
